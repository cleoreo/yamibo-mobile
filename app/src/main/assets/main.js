      ...
      if (/\b&fid=\b/.test(window.location.search) && !/\b&action=\b/.test(window.location.search)) {
        ...
        jQuery('.tl .bm_c .xg1').each(function () {
          ...
          if (dateText.split('回').length > 1) {
            newHtml += '<span class="no-of-reply"><i class="fas fa-comment"></i>' + dateText.split('回')[1] + '</span>';
          } else {
            newHtml += '<span class="no-of-reply"><i class="fas fa-comment"></i>0</span>';
          }
          newHtml += '</div>';
